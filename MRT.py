...
class MRT:
	...
	def mrt_open(self, host, port):
		try:
			...
			print("Server binded to {0}. Now ready to connect or receive connections...".format((host,port)))
			return True
		except:
			raise
	...
	def mrt_broadcast(self, msg):
		msg_to_broadcast = "{0}{1}".format('BCST', msg)
				...
				if verify_checksum(data):
					if addr not in self.connections: #if data not from connected sender, check if requesting connection
						...
						if packet_type == 'DATA': #packet containing data of shared-file
							...
							self.send_peers()
						elif packet_type == 'BCST': #packet containing user-inputed text for chatting.
